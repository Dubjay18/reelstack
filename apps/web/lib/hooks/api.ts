...
export function useUpdateListItem(listId: string) {
  ...
  return useMutation({
    ...
    // Implement Optimistic Updates for premium UX
...
// 6. Embed Hooks
export function useEmbedList(username: string, slug: string) {
  return useQuery({
    queryKey: ['embed', username, slug],
    queryFn: () => api.get<{ list: List; items: ListItem[] }>(`/api/v1/embed/${username}/${slug}`),
